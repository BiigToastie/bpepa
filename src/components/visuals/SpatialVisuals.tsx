...
function FoldedPaper({ folds }: { folds: number }) {
  ...
  return (
    <ShapeSvg w={120} h={120} pad={16}>
      ...
      <text x="60" y="112" textAnchor="middle" fill="#9499b8" fontSize="10">{sections} Lagen</text>
    </ShapeSvg>
  )
}
...
function CubeRotation({ degrees, axis }: { degrees: number; axis: 'vertical' | 'horizontal' }) {
  return (
    <ShapeSvg w={120} h={115} pad={18}>
      ...
      <path d="M 95 20 A 18 18 0 0 1 110 38" fill="none" stroke="#43e97b" strokeWidth="2" markerEnd="url(#cube-rot-arrow)" />
      <defs>
        <marker id="cube-rot-arrow" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
          <path d="M0,0 L6,3 L0,6 Z" fill="#43e97b" />
        </marker>
      </defs>
      <text x="60" y="108" textAnchor="middle" fill="#9499b8" fontSize="10">
...
function SphereShadow() {
  return (
    <ShapeSvg w={120} h={110} pad={16}>
      <defs>
        <radialGradient id="sphereGrad2" cx="35%" cy="30%">
          <stop offset="0%" stopColor="#a8b4ff" />
          <stop offset="100%" stopColor="#4a5fc1" />
        </radialGradient>
        <marker id="sphere-light" markerWidth="6" markerHeight="6" refX="3" refY="3">
          <circle cx="3" cy="3" r="3" fill="#fee140" />
        </marker>
      </defs>
      <line x1="60" y1="5" x2="60" y2="30" stroke="#fee140" strokeWidth="2" markerEnd="url(#sphere-light)" />
...
function ArrowRotation() {
  return (
    <ShapeSvg w={100} h={100}>
      ...
      <text x="50" y="88" textAnchor="middle" fill="#9499b8" fontSize="10">Start: nach rechts</text>
    </ShapeSvg>
  )
}
...
function RectangleArea({ length, width }: { length?: number; width?: number }) {
  ...
  return (
    <ShapeSvg w={160} h={105}>
      <rect x="25" y="20" width="110" height="55" fill="#43e97b" opacity="0.2" stroke="#43e97b" strokeWidth="2" rx="2" />
      <text x="80" y="53" textAnchor="middle" fill="#f4f6ff" fontSize="13" fontWeight="600">{label}</text>
      <text x="80" y="92" textAnchor="middle" fill="#9499b8" fontSize="10">Rechteckige Fläche</text>
    </ShapeSvg>
  )
}
